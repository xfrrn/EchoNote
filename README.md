...
本仓库为 **Monorepo**，目前包含已可运行的 Web 前端（高保真 PWA Demo）与完成 Phase 6 Search 的 Go 后端。
...
`apps/server` 已完成基础设施、Apple Podcasts/RSS/直接音频导入、Episode Library、Capture/Notes、异步 Transcription/Transcript/Speaker，以及 Notes + Transcript 的关键词、向量和 Hybrid Search。AI 与 Export 仍按后续 Vertical Slice 实现；Web Demo 暂未从 Mock Data 切换到后端。
