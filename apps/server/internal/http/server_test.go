...
func TestHealthEndpoints(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			router := NewRouter(pingerFunc(func(context.Context) error { return test.pingError }), nil, nil, nil, nil, nil, false, pgtype.UUID{}, logger)
...
func TestImportRejectsInvalidInputBeforeDatabase(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	router := NewRouter(pingerFunc(func(context.Context) error { return nil }), nil, nil, nil, nil, nil, false, pgtype.UUID{}, logger)
	tests := []struct {
		method string
		path   string
		body   string
