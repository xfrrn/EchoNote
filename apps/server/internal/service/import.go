...
func NewResolveImportHandler(imports *repository.ImportRepository, resolver domain.EpisodeResolver) worker.Handler {
	return func(ctx context.Context, job db.Job) error {
		...
		_, err = imports.SaveResolved(ctx, job.UserID, job.EntityID, status.SubmittedUrl, resolved)
		return err
	}
}
